...
class Calendar(commands.Cog):
    ...
    def __init__(self, bot: commands.Bot):
        ...
        self.bot = bot
        self.reminders = self.bot.config.get('reminders', [])
        self.calendar_message_id = 1320865537135349762
        self.calendar_channel_id = 1293319532361809986
    ...
    async def calendar(self, interaction: Interaction, option: app_commands.Choice[str], course: str, date: str, event: str, description: str = None, modality: str = None):
        ...
        try:
            ...
            calendar_channel = interaction.guild.get_channel(self.calendar_channel_id)
            msg = None

            if self.calendar_message_id:
                try:
                    msg = await calendar_channel.fetch_message(self.calendar_message_id)
                except NotFound:
                    pass
            ...
            if option.value == "1":
                ...
                if msg:
                    ...
                    embed = Embed(title=course.upper())
                    embed.add_field(name=f'__{event}__', value=(description + "\n\n" if description else "") + f'Echéance: {reminder_timestamp}' + ("\n\n``" + modality + '``' if modality else ""), inline=False)
                    msg = await calendar_channel.send(embeds=[embed])
